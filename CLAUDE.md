...
- The model dropdown (chat + Manus Claw) is database-backed, not `models.json`: an admin registers models via **Settings > Models**, with credentials encrypted at rest. See "Model Registry" in [AGENTS.md](AGENTS.md) for `MODEL_ENCRYPTION_KEY` and the one-time importer for upgrading an existing deployment.
