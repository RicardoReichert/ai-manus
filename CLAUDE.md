...
1. **`domain/services/flows/plan_act.py`** — `PlanActFlow.run()` is a state machine: `IDLE → PLANNING → EXECUTING → UPDATING → (repeat) → SUMMARIZING → COMPLETED`. Planner creates/updates the plan; Executor runs **one step at a time** then `complete_step`. It constructs the toolkit list (Shell, Browser, File, Message, Skill, MCP, optional Search) and drives two agents. (`agent_loop.py` remains as the experimental single-loop alternative, not wired by default.)
2. **`domain/services/agents/`** — `PlannerAgent` (`planner.py`) creates/updates the plan; `ExecutionAgent` (`execution.py`) runs each step. Both extend `BaseAgent` (`base.py`), which wraps LangChain's `init_chat_model`, handles tool-call parsing with retry/repair (`domain/utils/robust_json_parser.py`), memory compaction, and iteration limits. A model whose registry entry uses the `lean` tool profile also gets `WebAgent` (`web.py`): an isolated sub-agent holding only the browser toolkit, invoked through `DelegationToolkit`'s single `browse_web` tool so the supervising model never sees the 12-tool browser surface directly (see `domain/services/tools/profiles.py`) — kept off by default (`full` profile) so this is opt-in per model, not a behavior change for existing ones.
...
- The model dropdown is database-backed, not `models.json`: an admin registers models via **Settings > Models**, with credentials encrypted at rest. See "Model Registry" in [AGENTS.md](AGENTS.md) for `MODEL_ENCRYPTION_KEY` and the one-time importer for upgrading an existing deployment.
