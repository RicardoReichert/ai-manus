export default {
  'Hello': 'Hello',
  'What can I do for you?': 'What can I do for you?',
  // Home suggestion chips
  'Create slides': 'Create slides',
  'Build website': 'Build website',
  'Design': 'Design',
  'Create games': 'Create games',
  'Deep research': 'Deep research',
  'Analyze data': 'Analyze data',
  'Generate image': 'Generate image',
  'Write report': 'Write report',
  'Create spreadsheet': 'Create spreadsheet',
  'Develop apps': 'Develop apps',
  'Scheduled task': 'Scheduled task',
  'Wide Research': 'Wide Research',
  'Visualization': 'Visualization',
  'Chat mode': 'Chat mode',
  'Playbook': 'Playbook',
  'More': 'More',
  'Collapse sidebar': 'Collapse sidebar',
  'Expand sidebar': 'Expand sidebar',
  'Failed to create agent, please try again later': 'Failed to create agent, please try again later',
  'New Chat': 'New Chat',
  'New Task': 'New Task',
  'Thinking': 'Thinking',
  '{name} is thinking': '{name} is thinking',
  'Diff': 'Diff',
  'Original': 'Original',
  'Modified': 'Modified',
  'Task Progress': 'Task Progress',
  'Task progress': 'Task progress',
  'Task Completed': 'Task Completed',
  '{product} will continue after your reply': '{product} will continue after your reply',
  'Task completed': 'Task completed',
  'Copy': 'Copy',
  'Copied': 'Copied',
  'Collapse task progress': 'Collapse task progress',
  'Expand task progress': 'Expand task progress',
  "Use {product}'s computer": "Use {product}'s computer",
  "Use application on {product}'s computer": "Use application on {product}'s computer",
  "You're about to use {product}'s computer. ": "You're about to use {product}'s computer. ",
  'When finished, please inform {product} of your changes to help it work effectively.': 'When finished, please inform {product} of your changes to help it work effectively.',
  'Use application': 'Use application',
  'Create a task to get started': 'Create a task to get started',
  'All Tasks': 'All Tasks',
  'Tasks': 'Tasks',
  'Delete': 'Delete',
  'Search': 'Search',
  'Search tasks...': 'Search tasks...',
  'Search for task': 'Search for task',
  'Today': 'Today',
  'Yesterday': 'Yesterday',
  'Past 7 days': 'Past 7 days',
  'Past 30 days': 'Past 30 days',
  'Older': 'Older',
  'Library': 'Library',
  'Projects': 'Projects',
  'Agent': 'Agent',
  'Chat': 'Chat',
  'Failed to update mode': 'Failed to update mode',
  'Plugins': 'Plugins',
  'Scheduled tasks': 'Scheduled tasks',
  'Filter': 'Filter',
  'Desktop': 'Desktop',
  'Notifications': 'Notifications',
  'Coming soon': 'Coming soon',
  'New project': 'New project',
  'Favorites': 'Favorites',
  'Shared': 'Shared',
  'Non-project tasks': 'Non-project tasks',
  'No matching tasks': 'No matching tasks',
  'No favorite tasks': 'No favorite tasks',
  'No shared tasks': 'No shared tasks',
  'No non-project tasks': 'No non-project tasks',
  'Data Controls': 'Data Controls',
  'Archived': 'Archived',
  'Apps': 'Apps',
  'Purchased domains': 'Purchased domains',
  'No archived tasks': 'No archived tasks',
  'Not available yet': 'Not available yet',
  'Unshare': 'Unshare',
  'Restore': 'Restore',
  'Link unshared': 'Link unshared',
  'Failed to unshare session': 'Failed to unshare session',
  'Task restored': 'Task restored',
  'No tasks in project': 'No tasks in this project',
  'No files in library': 'No files in library',
  'No favorite files': 'No favorite files',
  'All': 'All',
  'My favorites': 'My favorites',
  'Locate in task': 'Locate in task',
  'Locate in new tab': 'Locate in new tab',
  'Visit': 'Visit',
  'Preview': 'Preview',
  'Send to Manus': 'Send to Manus',
  'Failed to send file to Manus': 'Failed to send file to Manus',
  'Search files': 'Search files',
  'Grid view': 'Grid view',
  'List view': 'List view',
  'Slides': 'Slides',
  'Sites': 'Sites',
  'Documents': 'Documents',
  'Spreadsheets': 'Spreadsheets',
  'Images': 'Images',
  'Audio & Video': 'Audio & Video',
  'Others': 'Others',
  'Untitled task': 'Untitled task',
  '{count} more files': '{count} more files',
  'Library coming soon': 'Library coming soon',
  'Projects coming soon': 'Projects coming soon',
  'More options': 'More options',
  'Rename': 'Rename',
  'Add to favorites': 'Add to favorites',
  'Unfavorite': 'Unfavorite',
  'Open in new tab': 'Open in new tab',
  'Delete this task?': 'Delete this task?',
  'Enter task name': 'Enter task name',
  'Enter project name': 'Enter project name',
  'Save': 'Save',
  'Create': 'Create',
  'Renamed successfully': 'Renamed successfully',
  'Failed to rename session': 'Failed to rename session',
  'Failed to share session': 'Failed to share session',
  'Failed to update favorite': 'Failed to update favorite',
  'Failed to create project': 'Failed to create project',
  'Failed to update project': 'Failed to update project',
  'Failed to move session': 'Failed to move session',
  'Added to favorite': 'Added to favorite',
  'Removed from favorite': 'Removed from favorite',
  'Move to project': 'Move to project',
  'Remove from project': 'Remove from project',
  'Moved to project': 'Moved to project',
  'Removed from project': 'Removed from project',
  'Project created': 'Project created',
  'Project not found': 'Project not found',
  'Project updated successfully': 'Project updated successfully',
  'Project deleted': 'Project deleted',
  'Edit project': 'Edit project',
  'Edit instructions': 'Edit instructions',
  'View project': 'View project',
  'Delete project': 'Delete project',
  'Delete project?': 'Delete project?',
  'Tasks in this project will not be deleted, but they will be removed from the project.':
    'Tasks in this project will not be deleted, but they will be removed from the project.',
  'Project instructions': 'Project instructions',
  'Applies to all new messages in this project.': 'Applies to all new messages in this project.',
  'Guide how Manus responds across all tasks in this project.':
    'Guide how Manus responds across all tasks in this project.',
  'Add': 'Add',
  'No tasks yet': 'No tasks yet',
  'Create a new task to get started': 'Create a new task to get started',
  'Your tasks stay private unless shared': 'Your tasks stay private unless shared',
  'Instructions': 'Instructions',
  'Created by {name}': 'Created by {name}',
  'Updated {date}': 'Updated {date}',
  'Give Manus a task...': 'Give Manus a task...',
  'Keep responses concise and professional.\nUse our brand voice from the attached guidelines.\nAlways provide sources for important conclusions.\nNever publish without approval.':
    'Keep responses concise and professional.\nUse our brand voice from the attached guidelines.\nAlways provide sources for important conclusions.\nNever publish without approval.',
  'New task': 'New task',
  'Failed to create task': 'Failed to create task',
  'Edit': 'Edit',
  'Expand': 'Expand',
  'Collapse': 'Collapse',
  'Pin': 'Pin',
  'Unpin': 'Unpin',
  'Pinned': 'Pinned',
  'Project pinned': 'Project pinned',
  'Project unpinned': 'Project unpinned',
  'Task pinned': 'Task pinned',
  'Task unpinned': 'Task unpinned',
  'Failed to update pin': 'Failed to update pin',
  'Archive task': 'Archive task',
  'Task archived': 'Task archived',
  'Failed to archive task': 'Failed to archive task',
  'Loading...': 'Loading...',
  'Untitled': 'Untitled',
  'Just now': 'Just now',
  'minutes ago': 'minutes ago',
  'hours ago': 'hours ago',
  'days ago': 'days ago',
  'months ago': 'months ago',
  'years ago': 'years ago',
  // Weekdays
  'Sunday': 'Sun',
  'Monday': 'Mon',
  'Tuesday': 'Tue',
  'Wednesday': 'Wed',
  'Thursday': 'Thu',
  'Friday': 'Fri',
  'Saturday': 'Sat',
  'Manus Computer': 'Manus Computer',
  "Manus's Computer": "Manus's Computer",
  "{name}'s computer": "{name}'s computer",
  "{name}'s computer is inactive": "{name}'s computer is inactive",
  'Manus is using': 'Manus is using',
  'Waiting for instructions': 'Waiting for instructions',
  'Jump to live': 'Jump to live',
  'live': 'live',
  'Live': 'Live',
  'Close': 'Close',
  'Terminal session ended': 'Terminal session ended',
  'Select an application to use': 'Select an application to use',
  'Failed to load file content': 'Failed to load file content',
  'Give Manus a task to work on...': 'Give Manus a task to work on...',
  'Assign a task or type / to see more': 'Assign a task or type / to see more',
  'Send message to Manus': 'Send message to Manus',
  'Add files and more': 'Add files and more',
  'Add local files': 'Add local files',
  'From Library': 'From Library',
  'Add from Library': 'Add from Library',
  'Recent Tasks': 'Recent Tasks',
  'Plan (Ctrl+/)': 'Plan (Ctrl+/)',
  'Regarding task "{title}": ': 'Regarding task "{title}": ',
  'You can choose to take over the browser to complete the operation.': 'You can choose to take over the browser to complete the operation.',
  'Center view': 'Center view',
  'Side view': 'Side view',
  'Full page': 'Full page',
  'Side': 'Side',
  'Center': 'Center',
  'View': 'View',
  'Edit default view': 'Edit default view',
  'Previous': 'Previous',
  'Next': 'Next',
  'Play': 'Play',
  'Pause': 'Pause',
  'Only me': 'Only me',
  'Public access': 'Public access',
  'Anyone with a link can view': 'Anyone with a link can view',
  'Viewable by yourself only': 'Viewable by yourself only',
  'Copy link': 'Copy link',
  'Link copied': 'Link copied',
  'Take control': 'Take control',
  'View all files in this task': 'View all files in this task',
  'Share privacy tip': "Don't share personal information or third-party content without permission.",
  'Assign a task or ask anything': 'Assign a task or ask anything',
  // Home page suggestion chips
  'Create slides prompt': 'Create a slide deck about ',
  'Build website prompt': 'Build a website for ',
  'Design prompt': 'Design a ',
  'Create games prompt': 'Create a browser game about ',
  'Analyze data prompt': 'Analyze this data and share key insights: ',
  'Research prompt': 'Do in-depth research on ',
  'Write report prompt': 'Write a detailed report about ',
  'Create spreadsheet prompt': 'Create a spreadsheet to track ',
  'Develop apps prompt': 'Develop an app that ',
  'Video prompt': 'Create a video about ',
  'Scheduled task prompt': 'Set up a recurring task that ',
  'Wide Research prompt': 'Run a wide, multi-source research on ',
  'Visualization prompt': 'Create a data visualization of ',
  'Audio prompt': 'Create an audio narration about ',
  'Playbook prompt': 'Create a reusable playbook for ',
  // Shell tools
  'Executing command': 'Executing command',
  'Viewing command output': 'Viewing command output',
  'Waiting for command completion': 'Waiting for command completion',
  'Writing data to process': 'Writing data to process',
  'Terminating process': 'Terminating process',
  // File tools
  'Reading file': 'Reading file',
  'Writing file': 'Writing file',
  'Replacing file content': 'Replacing file content',
  'Searching file content': 'Searching file content',
  'Finding file': 'Finding file',
  // Browser tools
  'Viewing webpage': 'Viewing webpage',
  'Navigating to webpage': 'Navigating to webpage',
  'Restarting browser': 'Restarting browser',
  'Clicking element': 'Clicking element',
  'Entering text': 'Entering text',
  'Moving mouse': 'Moving mouse',
  'Pressing key': 'Pressing key',
  'Selecting option': 'Selecting option',
  'Scrolling up': 'Scrolling up',
  'Scrolling down': 'Scrolling down',
  'Executing JS code': 'Executing JS code',
  'Viewing console output': 'Viewing console output',
  // Search tools
  'Searching web': 'Searching web',
  // Message tools
  'Sending notification': 'Sending notification',
  'Asking question': 'Asking question',
  // Delegation tool (Lean tool profile)
  'Browsing web': 'Browsing web',
  // Tool names
  'Terminal': 'Terminal',
  'No tool activity yet': 'No tool activity yet',
  'Done': 'Done',
  'File': 'File',
  'Browser': 'Browser',
  'Information': 'Information',
  'Message': 'Message',
  // Dialog
  'Confirm': 'Confirm',
  'Cancel': 'Cancel',
  'Close Dialog': 'Close Dialog',
  'Are you sure you want to delete this session?': 'Are you sure you want to delete this session?',
  'The chat history of this session cannot be recovered after deletion.': 'The chat history of this session cannot be recovered after deletion.',
  'Deleted successfully': 'Deleted successfully',
  'Failed to delete session': 'Failed to delete session',
  'Are you sure you want to delete Claw?': 'Are you sure you want to delete Claw?',
  'Chat history will be cleared. The Claw instance will remain available for re-creation.': 'Chat history will be cleared. The Claw instance will remain available for re-creation.',
  'Exit Takeover': 'Exit Takeover',
  'Take Over': 'Take Over',
  // Session File List
  'All Files in This Task': 'All Files in This Task',
  'No Content': 'No Content',
  'Failed to load files': 'Failed to load files',
  // File Preview
  'This format cannot be previewed': 'This format cannot be previewed',
  'Preview is not available for this format.': 'Preview is not available for this format.',
  'Please download the file to view its content': 'Please download the file to view its content',
  'Please download the file to view its contents.': 'Please download the file to view its contents.',
  'Zoom in': 'Zoom in',
  'Zoom out': 'Zoom out',
  'Download': 'Download',
  // File Upload
  'Upload failed': 'Upload failed',
  'Uploading...': 'Uploading...',
  // File Types
  'Text': 'Text',
  'Markdown': 'Markdown',
  'Code': 'Code',
  'Image': 'Image',
  'PDF': 'PDF',
  'Word': 'Word',
  'Excel': 'Excel',
  'PowerPoint': 'PowerPoint',
  'Document': 'Document',
  'Video': 'Video',
  'Audio': 'Audio',
  'Archive': 'Archive',
  // Login/Register Page
  'Login to Manus': 'Login to Manus',
  'Register to Manus': 'Register to Manus',
  'Full Name': 'Full Name',
  'Email': 'Email',
  'Password': 'Password',
  'Confirm Password': 'Confirm Password',
  'Enter your full name': 'Enter your full name',
  'Enter password': 'Enter password',
  'Enter password again': 'Enter password again',
  'Processing...': 'Processing...',
  'Login': 'Login',
  'Register': 'Register',
  'Already have an account?': 'Already have an account?',
  'Don\'t have an account?': 'Don\'t have an account?',
  'Login successful! Welcome back': 'Login successful! Welcome back',
  'Registration successful! Welcome to Manus': 'Registration successful! Welcome to Manus',
  'Authentication failed, please try again': 'Authentication failed, please try again',
  'Passwords do not match': 'Passwords do not match',
  // Validation error messages
  'Full name must be at least 2 characters long': 'Full name must be at least 2 characters long',
  'Full name must be less than 100 characters': 'Full name must be less than 100 characters',
  'Please enter a valid email address': 'Please enter a valid email address',
  'Password must be at least 6 characters long': 'Password must be at least 6 characters long',
  // User Menu
  'Logout': 'Logout',
  'Unknown User': 'Unknown User',
  'No email': 'No email',
  // Settings Dialog
  'Settings': 'Settings',
  'Account': 'Account',
  'General': 'General',
  'Language': 'Language',
  'Profile': 'Profile',
  'Personal': 'Personal',
  'Features': 'Features',
  'Shortcuts': 'Shortcuts',
  'Personalization': 'Personalization',
  'Get help': 'Get help',
  'Home page': 'Home page',
  'Appearance': 'Appearance',
  'Theme': 'Theme',
  'Light': 'Light',
  'Dark': 'Dark',
  'Auto': 'Auto',
  'Communication preferences': 'Communication preferences',
  'Browser notifications': 'Browser notifications',
  "Get notified in your browser when there's new progress or a task is completed.": "Get notified in your browser when there's new progress or a task is completed.",
  'Sound alert': 'Sound alert',
  'Play a sound once when a task is completed while you are away.': 'Play a sound once when a task is completed while you are away.',
  'Full name': 'Full name',
  'User ID': 'User ID',
  'Failed to copy': 'Failed to copy',
  'Sign out of your account on this device.': 'Sign out of your account on this device.',
  'Log out of all devices': 'Log out of all devices',
  'Sign out of your account everywhere, including other browsers and devices.': 'Sign out of your account everywhere, including other browsers and devices.',
  'Log out everywhere': 'Log out everywhere',
  'This will sign you out everywhere, including other browsers and devices. You will need to log in again.': 'This will sign you out everywhere, including other browsers and devices. You will need to log in again.',
  'Click a shortcut to record a new key combination for it.': 'Click a shortcut to record a new key combination for it.',
  'Press a key combination...': 'Press a key combination...',
  'Shortcuts need at least one modifier key (Ctrl/Cmd/Alt)': 'Shortcuts need at least one modifier key (Ctrl/Cmd/Alt)',
  'Reset': 'Reset',
  'Nickname': 'Nickname',
  'Manage your identity information for Manus.': 'Manage your identity information for Manus.',
  'Manus uses this information to personalize responses.': 'Manus uses this information to personalize responses.',
  'Find documentation and report issues.': 'Find documentation and report issues.',
  'Documentation': 'Documentation',
  'GitHub Issues': 'GitHub Issues',
  // Profile Settings
  'Name': 'Name',
  'Full name updated successfully': 'Full name updated successfully',
  'Change photo': 'Change photo',
  'Remove photo': 'Remove photo',
  'Adjust photo': 'Adjust photo',
  'Zoom': 'Zoom',
  'Profile photo updated': 'Profile photo updated',
  'Profile photo removed': 'Profile photo removed',
  'Failed to update profile photo': 'Failed to update profile photo',
  'Failed to remove profile photo': 'Failed to remove profile photo',
  'Please choose a JPEG, PNG, or WebP image.': 'Please choose a JPEG, PNG, or WebP image.',
  'Image must be 5MB or smaller.': 'Image must be 5MB or smaller.',
  'Failed to update full name': 'Failed to update full name',
  'Update Password': 'Update Password',
  // Change Password Dialog
  'Current Password': 'Current Password',
  'New Password': 'New Password',
  'Confirm New Password': 'Confirm New Password',
  'Enter current password': 'Enter current password',
  'Enter new password': 'Enter new password',
  'Enter new password again': 'Enter new password again',
  'Password change successful': 'Password change successful',
  'Password change failed': 'Password change failed',
  'Forgot Password?': 'Forgot Password?',
  'Reset Password': 'Reset Password',
  'Reset link sent!': 'Reset link sent!',
  'Please check your email for further instructions.': 'Please check your email for further instructions.',
  'Remember your password?': 'Remember your password?',
  'Back to Login': 'Back to Login',
  'Reset link sent to your email': 'Reset link sent to your email',
  'Failed to send reset link. Please try again.': 'Failed to send reset link. Please try again.',
  // Language Selection
  'Select language': 'Select language',
  'Simplified Chinese': 'Simplified Chinese',
  'English': 'English',
  'Portuguese (Brazil)': 'Portuguese (Brazil)',
  // MCP Tool View
  'Tool': 'Tool',
  'Arguments': 'Arguments',
  'Result': 'Result',
  'Exit code': 'Exit code',
  'Output': 'Output',
  'Output truncated': 'Output truncated',
  'Tool is executing...': 'Tool is executing...',
  'Waiting for result...': 'Waiting for result...',
  // Reset Password Verification
  'Verification code sent to': 'Verification code sent to',
  'Enter 6-digit verification code': 'Enter 6-digit verification code',
  'Resend': 'Resend',
  'Verification code is required': 'Verification code is required',
  'Verification code must be 6 digits': 'Verification code must be 6 digits',
  'Please confirm your password': 'Please confirm your password',
  'Enter your new password': 'Enter your new password',
  'Confirm your new password': 'Confirm your new password',
  'Updating...': 'Updating...',
  'Password updated successfully!': 'Password updated successfully!',
  'You can now login with your new password.': 'You can now login with your new password.',
  'Didn\'t receive the code?': 'Didn\'t receive the code?',
  'Resend in {seconds}s': 'Resend in {seconds}s',
  'Resend Code': 'Resend Code',
  'Ready to login?': 'Ready to login?',
  'Want to try a different email?': 'Want to try a different email?',
  'Go Back': 'Go Back',
  'Verification code sent again': 'Verification code sent again',
  'Failed to resend verification code. Please try again.': 'Failed to resend verification code. Please try again.',
  'Password updated successfully': 'Password updated successfully',
  'Failed to update password. Please check your verification code and try again.': 'Failed to update password. Please check your verification code and try again.',
  'Verification code sent to your email': 'Verification code sent to your email',
  'Failed to send verification code. Please try again.': 'Failed to send verification code. Please try again.',
  'Invalid or expired verification code. Please try again.': 'Invalid or expired verification code. Please try again.',
  'User not found. Please check your email address.': 'User not found. Please check your email address.',
  'Invalid request. Please check your input and try again.': 'Invalid request. Please check your input and try again.',
  'Failed to update password. Please try again.': 'Failed to update password. Please try again.',
  'Login failed, please try again': 'Login failed, please try again',
  'Registration failed, please try again': 'Registration failed, please try again',
  'Session not found': 'Session not found',
  'Failed to create session, please try again later': 'Failed to create session, please try again later',
  // Reset Password Form
  'Sending Code...': 'Sending Code...',
  'Send Verification Code': 'Send Verification Code',
  // Share Feature
  'Share': 'Share',
  'Private Only': 'Private Only',
  'Only visible to you': 'Only visible to you',
  'Public Access': 'Public Access',
  'Anyone with the link can view': 'Anyone with the link can view',
  'Share Instantly': 'Share Instantly',
  'Sharing...': 'Sharing...',
  'Failed to change sharing settings': 'Failed to change sharing settings',
  'Copy Link': 'Copy Link',
  'Link Copied': 'Link Copied',
  'Link copied to clipboard': 'Link copied to clipboard',
  'Failed to copy link': 'Failed to copy link',
  'You are viewing a completed Manus task. Replay will start automatically in {countdown} seconds.': 'You are viewing a completed Manus task. Replay will start automatically in {countdown} seconds.',
  'Manus task replay completed.': 'Manus task replay completed.',
  'Manus is replaying the task...': 'Manus is replaying the task...',
  'Replay': 'Replay',
  'Jump to result': 'Jump to result',
  // Claw Page
  'Deploy OpenClaw Instantly': 'Deploy OpenClaw Instantly',
  'OpenClaw is an AI assistant with unique personality and long-term memory, one-click deploy to sandbox cloud, no complex setup, 24/7 online': 'OpenClaw is an AI assistant with unique personality and long-term memory, one-click deploy to sandbox cloud, no complex setup, 24/7 online',
  'Chat Freely via Manus': 'Chat Freely via Manus',
  'Auto-configured with powerful LLMs and skill libraries, supports multiple chat tools, proactively completes various tasks': 'Auto-configured with powerful LLMs and skill libraries, supports multiple chat tools, proactively completes various tasks',
  'Get Started': 'Get Started',
  'Create Manus Claw': 'Create Manus Claw',
  'One-click OpenClaw deployment by Manus': 'One-click OpenClaw deployment by Manus',
  'Creating...': 'Creating...',
  'Claw is initializing, please wait...': 'Claw is initializing, please wait...',
  'Claw is ready, let\'s chat!': 'Claw is ready, let\'s chat!',
  'Claw has stopped': 'Claw has stopped',
  'Recreate': 'Recreate',
  'Claw failed to start': 'Claw failed to start',
  'Claw failed to start, please recreate.': 'Claw failed to start, please recreate.',
  'Claw has stopped, please recreate to continue chatting.': 'Claw has stopped, please recreate to continue chatting.',
  'Delete and recreate': 'Delete and recreate',
  'An error occurred': 'An error occurred',
  'Creation failed, please try again later': 'Creation failed, please try again later',
  'Please check {count} attachment(s) I sent': 'Please check {count} attachment(s) I sent',
  'WebSocket not connected, please try again later': 'WebSocket not connected, please try again later',
  'Claw has expired, please recreate.': 'Claw has expired, please recreate.',

  // Model switcher, task logs drawer, action logs
  'Available Models': 'Available Models',
  'Task Execution Logs': 'Task Execution Logs',
  'Task Logs & Context': 'Task Logs & Context',
  'User message': 'User message',
  'Assistant message': 'Assistant message',
  'Plan updated ({n} steps)': 'Plan updated ({n} steps)',
  'Error': 'Error',
  'Title set': 'Title set',
  'Waiting for input': 'Waiting for input',
  'Task finished': 'Task finished',
  'Usage': 'Usage',
  'Time worked': 'Time worked',
  'Pages viewed': 'Pages viewed',
  'Commands run': 'Commands run',
  'APIs called': 'APIs called',
  'Files created': 'Files created',
  'Rate this task': 'Rate this task',
  'Failed to update rating': 'Failed to update rating',
  'Model & Execution Environment': 'Model & Execution Environment',
  'Active Model': 'Active Model',
  'Task Mode': 'Task Mode',
  'Execution Timeline': 'Execution Timeline',
  'No execution logs recorded yet.': 'No execution logs recorded yet.',
  'Workspace Files': 'Workspace Files',
  'No files generated in this task.': 'No files generated in this task.',
  'Local': 'Local',
  'Applies to your next message': 'Applies to your next message',
  'Failed to switch model': 'Failed to switch model',
  'Copy parameters': 'Copy parameters',
  'Loading files…': 'Loading files…',
  // Model registry admin (Settings > Models, Claw model picker)
  'Models': 'Models',
  'Admin': 'Admin',
  'Add model': 'Add model',
  'Edit model': 'Edit model',
  'Provider': 'Provider',
  'Display name': 'Display name',
  'Registry ID': 'Registry ID',
  'Used internally; cannot be changed after creation. Letters, numbers, \'.\', \'_\', \'-\' only — it becomes part of a URL, so it cannot contain \'/\'.': 'Used internally; cannot be changed after creation. Letters, numbers, \'.\', \'_\', \'-\' only — it becomes part of a URL, so it cannot contain \'/\'.',
  'Model name': 'Model name',
  'The exact name the provider expects.': 'The exact name the provider expects.',
  'Base URL': 'Base URL',
  'Leave empty for the provider default.': 'Leave empty for the provider default.',
  'API key': 'API key',
  'Leave blank for local models that need no authentication.': 'Leave blank for local models that need no authentication.',
  'A key is stored (ends in {hint}). Leave blank to keep it.': 'A key is stored (ends in {hint}). Leave blank to keep it.',
  'Tool profile': 'Tool profile',
  'Lean gives the model shell/file/message/search plus delegated browsing — best for small (~4B) models.': 'Lean gives the model shell/file/message/search plus delegated browsing — best for small (~4B) models.',
  'Full': 'Full',
  'Lean': 'Lean',
  'Enabled (visible in the model dropdown)': 'Enabled (visible in the model dropdown)',
  'Remove stored API key': 'Remove stored API key',
  'Saving...': 'Saving...',
  'Model added': 'Model added',
  'Model updated': 'Model updated',
  'Model deleted': 'Model deleted',
  'Failed to load models': 'Failed to load models',
  'Failed to save model': 'Failed to save model',
  'Failed to delete model': 'Failed to delete model',
  'Failed to update model': 'Failed to update model',
  'Delete this model?': 'Delete this model?',
  'It will no longer appear in the model dropdown. Sessions already using it will show an error until switched.': 'It will no longer appear in the model dropdown. Sessions already using it will show an error until switched.',
  'No models configured — add the first one.': 'No models configured — add the first one.',
  'Models registered here appear in the chat and Manus Claw model dropdowns for every user. Credentials are encrypted and never shown again after saving.': 'Models registered here appear in the chat and Manus Claw model dropdowns for every user. Credentials are encrypted and never shown again after saving.',
  'MODEL_ENCRYPTION_KEY is not configured on the server, so models with an API key cannot be saved. Local models with no credential (LM Studio, Ollama) are unaffected.': 'MODEL_ENCRYPTION_KEY is not configured on the server, so models with an API key cannot be saved. Local models with no credential (LM Studio, Ollama) are unaffected.',
  'Disabled': 'Disabled',
  'Small-model profile': 'Small-model profile',
  'Tool count is limited for this model, and browsing is delegated to a sub-agent.': 'Tool count is limited for this model, and browsing is delegated to a sub-agent.',
  'Test connection': 'Test connection',
  'Testing...': 'Testing...',
  'Test failed': 'Test failed',
  'Connected ({ms} ms)': 'Connected ({ms} ms)',
  'e.g. GPT-4o': 'e.g. GPT-4o',
  // Model registry admin follow-ups (refresh, description field)
  'Description': 'Description',
  'Shown under the model name in the dropdown. Leave empty to show nothing.': 'Shown under the model name in the dropdown. Leave empty to show nothing.',
  'Optional': 'Optional',
  // Model registry admin: tool picker, capabilities display, local toggle
  'Auto profile tooltip': 'Auto: resolves to Lean or Full per-model from its own detected capabilities — the default for a newly registered model, so a small model gets a sane toolset without anyone flipping a toggle.',
  'Full profile tooltip': 'Full: every tool available (~25) — shell, browser, files, messages, search. The default; matches what large models already handle well.',
  'Lean profile tooltip': 'Lean: shell/files/messages/search plus one delegated browsing tool, instead of the browser toolkit directly. Keeps the tool count low for small (~4B) models, without losing the ability to browse.',
  'Tools': 'Tools',
  'By default the profile above decides. Customize to pick exactly which tools this model can call.': 'By default the profile above decides. Customize to pick exactly which tools this model can call.',
  'Customize tool list': 'Customize tool list',
  'Temperature override': 'Temperature override',
  'Temperature override hint': 'Sampling temperature for this model only. Leave blank to use the global default. Small/local models often need a lower value (e.g. 0.3) for reliable tool calling.',
  'Request timeout override': 'Request timeout override',
  'Request timeout override hint': 'Seconds to wait before giving up on a request to this model. Leave blank for no timeout. A local endpoint that hangs otherwise blocks the whole session.',
  'Temperature must be between 0 and 2.': 'Temperature must be between 0 and 2.',
  'Request timeout must be a positive number of seconds.': 'Request timeout must be a positive number of seconds.',
  'Detected capabilities': 'Detected capabilities',
  'Auto-detected from the model name; only informational here.': 'Auto-detected from the model name; only informational here.',
  'Tool budget: {n}': 'Tool budget: {n}',
  'No tool limit': 'No tool limit',
  'Guided decoding': 'Guided decoding',
  '{n} token context': '{n} token context',
  'Temperature {n}': 'Temperature {n}',
  'Timeout {n}s': 'Timeout {n}s',
  'Auto-detected': 'Auto-detected',
  'Manually set': 'Manually set',
  'Local model (runs on your network, e.g. LM Studio, Ollama)': 'Local model (runs on your network, e.g. LM Studio, Ollama)',
  // Manus Claw sessions (multi-session UI)
  'Choose a model': 'Choose a model',
  'New session': 'New session',
  'Restart': 'Restart',
  'Restarting with the new model — your conversation history is preserved.': 'Restarting with the new model — your conversation history is preserved.',
  'Running': 'Running',
  'Session {n}': 'Session {n}',
  'Sessions': 'Sessions',
  'Stopped': 'Stopped',
  'This permanently deletes its memory and cannot be undone. To just switch models, use the model dropdown instead — it restarts without losing history.': 'This permanently deletes its memory and cannot be undone. To just switch models, use the model dropdown instead — it restarts without losing history.',
  'This session’s container stopped. Its memory is preserved — restart to continue.': 'This session’s container stopped. Its memory is preserved — restart to continue.',

  // Claw Computer Panel
  'Computer': 'Computer',
}
