...
**Interfaces:**
- Consumes: Task 1's report (exact `session.tool` payload shape, confirmed operator scope).
...
- [ ] **Step 1:** Read Task 1's report file first — it has the confirmed `session.tool` payload shape and operator scope findings this task must use, not guess at.
- [ ] **Step 2:** Read `claw/manus-claw/src/gateway-bridge.js` in full (already partially described in this plan's Context section: `_handleAgentEvent` currently only handles `stream === 'assistant'` and `stream === 'lifecycle'`, lines ~99-156).
- [ ] **Step 3:** Add a branch handling the tool-event stream (using the exact `stream` value confirmed in Task 1's report) that extracts tool name/args/result/status from the payload (field names per Task 1's report) and forwards it to whatever mechanism `http-server.js` uses to push SSE events (read `_handleChat`/the SSE-writing code in `http-server.js` to find the existing pattern for `chunk`/`file`/`done`/`error` events and follow it exactly), emitting `{"type": "tool", "name": ..., "args": ..., "status": ..., ...}` (adapt field names to what's actually available from the real payload — do not invent fields that aren't in the source data).
- [ ] **Step 4:** In `claw/manus-claw/src/http-server.js`, add `POST /terminal/open`: calls the gateway's `terminal.open` operator RPC (using whatever client/connection object `gateway-bridge.js` already uses to talk to the gateway — read how existing RPCs, if any, are issued from this plugin, or how the gateway connection object is structured, to call `terminal.open` the same way) for the current agent/session, returns the resulting terminal session id to the caller.
- [ ] **Step 5:** Add a WS endpoint (e.g. `/terminal/{terminalSessionId}`) that: on client message, calls `terminal.input`/`terminal.resize` RPCs with the client's payload; on `terminal.data`/`terminal.exit` events from the gateway (scoped to this terminal session), forwards them out to the connected WS client. Handle `terminal.exit` by closing the WS cleanly. If Task 1 found the plugin's gateway connection lacks sufficient scope for these RPCs, this step must first address that (re-pairing, config change) before the endpoint can work — do not silently no-op on an RPC rejection; surface the error clearly in logs and in the task report.
